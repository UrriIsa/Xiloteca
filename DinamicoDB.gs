...
function onEdit(e) {
 ...
 //------------------------------------------------------------------- | GENERALES -------------------------------------------------
  ...
  /**
   * Definición de las columnas clave en la hoja de cálculo.
   */
  var columnaPais = 30; //si borras columnas debes verificar el numero
  ...
  if (columnaEditada === columnaPais || columnaEditada === columnaLocalidad || columnaEditada === columnaMunicipio) {
    ...
    if (valorPais === "México") {
      ...
      if (errorLocalidad) {
        ...
        celdaLocalidad.setBackground("white"); // Restaurar color
      }

      if (errorMunicipio) {
        celdaMunicipio.setBackground(colorIncorrecto); // Rojo
      } else {
        celdaMunicipio.setBackground("white"); // Restaurar color
      }
    } else {
      // Si el país NO es México, restaurar colores
      celdaLocalidad.setBackground("white");
      celdaMunicipio.setBackground("white");
    }
  }
}
